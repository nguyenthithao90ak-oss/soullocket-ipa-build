...
class PinnedHttpClient {
  PinnedHttpClient._(
      {required this.pinnedSpkiFingerprints, this.timeoutSeconds = 30});

  static final PinnedHttpClient defaultInstance = PinnedHttpClient._(
    pinnedSpkiFingerprints: _defaultFingerprints,
  );

  factory PinnedHttpClient.withFingerprints(Set<String> fingerprints,
  ...
  http.Client _createIoClient() {
    ...
    return http_io.IOClient(HttpClient(context: securityContext)
      ..badCertificateCallback = (X509Certificate cert, String host, int port) {
        ...
        if (fingerprint == null) {
          if (kDebugMode) debugPrint('[PinnedHttpClient] No SPKI for $host.');
          return false;
        }
  ...
  String? _spkiFingerprintOf(X509Certificate cert) {
    try {
      ...
      final derBytes =
          base64.decode(lines.where((l) => !l.startsWith('---')).join());
  ...
  Uint8List? _extractSpkiFromDer(Uint8List der) {
    try {
      ...
      if (reader.offset < tbsEnd && der[reader.offset] == 0xA0) {
        reader.skipTagged();
      }
      for (int i = 0; i < 5 && reader.offset < tbsEnd; i++) {
        reader.skipTlv();
      }
...
class _Asn1Reader {
  ...
  int readLength() {
    ...
    for (int i = 0; i < numOctets; i++) {
      if (offset >= _data.length) throw 'end';
      length = (length << 8) | (_data[offset++] & 0xFF);
    }
    return length;
  }
