...
function normalizeIpAddress(value) {
  const normalized = normalizeText(value).replace(/^::ffff:/i, '');
...
function normalizeUserAgentText(value) {
  return normalizeText(value).replace(/\s+/g, ' ').toLowerCase().slice(0, 400);
}

function pruneRegistrationAttempts(rawAttempts, now, windowMs) {
  const attempts = asObject(rawAttempts);
  return Object.entries(attempts)
    .map(([key, rawValue]) => {
      const value = asObject(rawValue);
      const ts = toTimestamp(value.ts);
      ...
      return [
        ...
        {
          ts,
          emailHash: normalizeText(value.emailHash),
          uid: normalizeText(value.uid),
          ipHash: normalizeText(value.ipHash),
          fingerprintHash: normalizeText(value.fingerprintHash),
          userAgentHash: normalizeText(value.userAgentHash),
...
async function applyRegistrationSignalLimit({
  db,
  signalType,
  signalHash,
  limit,
  ...
  const stateRef = db.ref(
    `auth_security/registration_limits/${signalType}/${signalHash}`,
  );
  let blocked = false;
  let shouldAudit = false;
  let retryAfterMs = 0;
  ...
  await stateRef.transaction((rawState) => {
    const state = asObject(rawState);
    const activeAttempts = pruneRegistrationAttempts(
      state.attempts,
      now,
    ...
    for (const [key, value] of activeAttempts.slice(-REGISTER_LIMIT_MAX_TRACKED_EVENTS)) {
      nextAttempts[key] = value;
    }

    const alreadyRecorded = Object.prototype.hasOwnProperty.call(
      nextAttempts,
      eventId,
    );
    const oldestTs = activeAttempts.length > 0 ? activeAttempts[0][1].ts : 0;
    activeCount = activeAttempts.length;
    blocked = !alreadyRecorded && activeCount >= limit;

    if (!blocked && !alreadyRecorded) {
      nextAttempts[eventId] = {
        ...attemptPayload,
        ts: now,
      };
      activeCount += 1;
    ...
    shouldAudit =
      blocked &&
      (lastAuditAt <= 0 || now - lastAuditAt >= REGISTER_LIMIT_AUDIT_COOLDOWN_MS);

    if (blocked && oldestTs > 0) {
      retryAfterMs = Math.max(REGISTER_LIMIT_WINDOW_MS - (now - oldestTs), 0);
    } else {
      retryAfterMs = 0;
    }
    ...
    return {
      ...
      lastEventId: eventId,
      lastUid: attemptPayload.uid,
      lastEmailHash: attemptPayload.emailHash,
      lastIpHash: attemptPayload.ipHash,
      lastFingerprintHash: attemptPayload.fingerprintHash,
      lastUserAgentHash: attemptPayload.userAgentHash,
      lastProviderId: attemptPayload.providerId,
      updatedAt: now,
      windowMs: REGISTER_LIMIT_WINDOW_MS,
      limit,
      lastStatus: blocked ? 'blocked' : 'allowed',
      ...(blocked
        ? {
          lastBlockedAt: now,
          lastRetryAfterMs: retryAfterMs,
          blockCount:
            Math.max(0, Math.trunc(Number(state.blockCount) || 0)) + 1,
          ...(shouldAudit ? { lastAuditAt: now } : {}),
        }
        : {
          lastAllowedAt: now,
...
async function logRegistrationAbuseEvent({
  user,
  signalType,
  activeCount,
  limit,
  ...
  const retryAfterDays = Math.max(
    1,
    Math.ceil(Math.max(0, retryAfterMs) / (24 * 60 * 60 * 1000)),
  );

  await admin.database().ref('admin_system/abuse_logs').push().set({
    type: 'abnormal',
    uid: normalizeText(user.uid) || 'pending_user',
    email: normalizeEmail(user.email),
    details:
      signalType === 'fingerprints'
        ? `Blocked create-account burst from same device/network fingerprint (${activeCount}/${limit} in 7d). Retry in ~${retryAfterDays} day(s).`
        : `Blocked create-account burst from same IP (${activeCount}/${limit} in 7d). Retry in ~${retryAfterDays} day(s).`,
    signalType,
    providerId,
    ipHash,
    fingerprintHash,
...
async function enforceRegistrationAbuseProtection(user, context) {
  const email = normalizeEmail(user.email);
  const ipAddress = normalizeIpAddress(context?.ipAddress);
  const userAgent = normalizeUserAgentText(context?.userAgent);
  const providerId = normalizeText(
    context?.credential?.providerId || context?.additionalUserInfo?.providerId,
  );
  ...
  const now = Date.now();
  const eventId =
    normalizeText(context?.eventId) || `evt_${now}_${sha256(email).slice(0, 10)}`;
  const emailHash = sha256(`email:${email || 'unknown'}`);
  const ipHash = sha256(`ip:${ipAddress}`);
  const userAgentHash = userAgent ? sha256(`ua:${userAgent}`) : '';
  const fingerprintSource = userAgent ? `${ipAddress}|${userAgent}` : ipAddress;
  const fingerprintHash = sha256(`fp:${fingerprintSource}`);
  const hasFingerprintSignal = Boolean(userAgentHash);
  ...
  if (hasFingerprintSignal) {
    const fingerprintResult = await applyRegistrationSignalLimit({
      db,
      signalType: 'fingerprints',
      signalHash: fingerprintHash,
      limit: REGISTER_LIMIT_FINGERPRINT_MAX,
      now,
      eventId,
      attemptPayload,
    });

    if (fingerprintResult.blocked) {
      if (fingerprintResult.shouldAudit) {
        await logRegistrationAbuseEvent({
          user,
          signalType: 'fingerprints',
          activeCount: fingerprintResult.activeCount,
          limit: REGISTER_LIMIT_FINGERPRINT_MAX,
          retryAfterMs: fingerprintResult.retryAfterMs,
          ipHash,
          fingerprintHash,
          userAgentHash,
          providerId,
      ...
      const retryAfterDays = Math.max(
        1,
        Math.ceil(fingerprintResult.retryAfterMs / (24 * 60 * 60 * 1000)),
      );
      throw new functions.https.HttpsError(
        'resource-exhausted',
        `Thiết bị/mạng này đã tạo quá nhiều tài khoản mới trong 7 ngày. Vui lòng thử lại sau khoảng ${retryAfterDays} ngày.`,
      );
    }
  }

  const ipResult = await applyRegistrationSignalLimit({
    db,
    signalType: 'ips',
    signalHash: ipHash,
    limit: REGISTER_LIMIT_IP_MAX,
    now,
    eventId,
    attemptPayload,
  });
  ...
  if (ipResult.blocked) {
    if (ipResult.shouldAudit) {
      await logRegistrationAbuseEvent({
        user,
        signalType: 'ips',
        activeCount: ipResult.activeCount,
        limit: REGISTER_LIMIT_IP_MAX,
    ...
    const retryAfterDays = Math.max(
      1,
      Math.ceil(ipResult.retryAfterMs / (24 * 60 * 60 * 1000)),
    );
    throw new functions.https.HttpsError(
      'resource-exhausted',
      `Mạng này đã tạo quá nhiều tài khoản mới trong 7 ngày. Vui lòng thử lại sau khoảng ${retryAfterDays} ngày.`,
    );
  }
}
...
function maskEmail(email) {
  ...
  return `${firstTwo}${'*'.repeat(Math.max(0, local.length - 4))}${lastTwo}@${domain}`;
}
...
function inferJoiningRole(members, ownerUid) {
  const normalizedOwnerUid = normalizeText(ownerUid);
  if (normalizedOwnerUid && Object.prototype.hasOwnProperty.call(members, normalizedOwnerUid)) {
    return 'user2';
  }

  for (const rawValue of Object.values(members)) {
    const item = asObject(rawValue);
    if (normalizeMemberRole(item.role) === 'user1') {
      return 'user2';
    }
  }

  return Object.keys(members).length === 0 ? 'user1' : 'user2';
}

function normalizeMemberPayload({ uid, raw, fallbackRole }) {
  const payload = asObject(raw);
  const normalizedRole = normalizeMemberRole(payload.role) || fallbackRole;
  return {
    uid,
    displayName: normalizeText(payload.displayName),
    photoURL: normalizeText(payload.photoURL),
    role: normalizedRole,
    joinedAt: payload.joinedAt || admin.database.ServerValue.TIMESTAMP,
  };
}
...
function normalizeGroupChatId(value) {
  ...
  return /^[A-Za-z0-9_-]{6,80}$/.test(normalized) ? normalized : '';
}

function buildBooleanMap(items) {
  const result = {};
  items.forEach((item) => {
...
exports.createGroupChatSecure = functions
  .runWith({ enforceAppCheck: true })
  .https.onCall(async (data, context) => {
    if (!context.auth?.uid) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'Đăng nhập để tạo nhóm chat.',
      );
    }

    const uid = normalizeText(context.auth.uid);
    const requestedHouseId = normalizeText(data?.houseId);
    const requestedGroupId = normalizeText(data?.groupId);
    const providedGroupId = normalizeGroupChatId(requestedGroupId);
    if (requestedGroupId && !providedGroupId) {
      throw new functions.https.HttpsError(
        'invalid-argument',
    ...
    const requestedName = normalizeMultilineText(data?.name || '', 50);
    const rawMemberHouseIds = Array.isArray(data?.memberHouseIds)
      ? data.memberHouseIds
      : [];
    const { houseId: creatorHouseId } = await resolveMemberHouse(uid, requestedHouseId);

    const uniqueHouseIds = [...new Set([
      creatorHouseId,
      ...rawMemberHouseIds.map((item) => normalizeText(item)),
    ].filter(Boolean))];

    if (uniqueHouseIds.length < 2) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Nhóm chat cần ít nhất 2 nhà.',
      );
    }

    if (uniqueHouseIds.length > 12) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Nhóm chat hiện hỗ trợ tối đa 12 nhà.',
      );
    }

    const db = admin.database();
    const groupId = providedGroupId || db.ref('groups').push().key;
    if (!groupId) {
      throw new functions.https.HttpsError(
        'internal',
        'Không thể tạo mã nhóm mới.',
      );
    }
    ...
    if (existingSnapshot.exists()) {
      const existingData = asObject(existingSnapshot.val());
      const existingMembers = asObject(existingData.members);
      if (!Object.prototype.hasOwnProperty.call(existingMembers, uid)) {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Bạn không có quyền dùng nhóm chat này.',
        );
      }
      ...
      return {
        ...
        alreadyExists: true,
        groupId,
        name: normalizeMultilineText(existingData.name || '', 50) || 'Nhóm chat',
        memberHouseIds: Object.keys(asObject(existingData.memberHouseIds)),
        createdAt: toTimestamp(existingData.createdAt),
        updatedAt: toTimestamp(existingData.updatedAt),
      };
    }

    const memberUids = new Set([uid]);
    const houseIndexUpdates = {};
    const userIndexUpdates = {};
    const resolvedMemberHouseIds = [];

    for (const memberHouseId of uniqueHouseIds) {
      const houseSnapshot = await db.ref(`houses/${memberHouseId}`).once('value');
      if (!houseSnapshot.exists()) {
        throw new functions.https.HttpsError(
          'not-found',
          `Không tìm thấy nhà ${memberHouseId}.`,
        );
      }

      if (memberHouseId !== creatorHouseId) {
        const [friendForwardSnapshot, friendBackwardSnapshot] = await Promise.all([
          db.ref(`friends/${creatorHouseId}/${memberHouseId}`).once('value'),
          db.ref(`friends/${memberHouseId}/${creatorHouseId}`).once('value'),
        ]);
        if (!friendForwardSnapshot.exists() && !friendBackwardSnapshot.exists()) {
          throw new functions.https.HttpsError(
            'permission-denied',
            'Chỉ có thể thêm những nhà đang là bạn bè vào nhóm chat.',
          );
        }
      }

      const houseData = asObject(houseSnapshot.val());
      resolvedMemberHouseIds.push(memberHouseId);
      extractHouseMemberUids(houseData).forEach((memberUid) => {
        if (!memberUid) return;
        memberUids.add(memberUid);
        userIndexUpdates[`users/${memberUid}/group_ids/${groupId}`] = true;
      });
      houseIndexUpdates[`houses/${memberHouseId}/group_ids/${groupId}`] = true;
    }

    const groupName = requestedName || `Nhóm ${resolvedMemberHouseIds.length} thành viên`;
    const now = Date.now();
    const updates = {
      [`groups/${groupId}`]: {
        name: groupName,
        houseId: creatorHouseId,
        createdByUid: uid,
        createdByHouseId: creatorHouseId,
        createdAt: admin.database.ServerValue.TIMESTAMP,
        updatedAt: admin.database.ServerValue.TIMESTAMP,
        memberHouseIds: buildBooleanMap(resolvedMemberHouseIds),
        members: buildBooleanMap([...memberUids]),
        lastMessage: {
          text: '',
          type: 'system',
          ts: now,
    ...
    return {
      ...
      groupId,
      name: groupName,
      memberHouseIds: resolvedMemberHouseIds,
      createdAt: now,
      updatedAt: now,
    };
  });

exports.createHouseSecure = functions
  .runWith({ enforceAppCheck: true })
  .https.onCall(async (data, context) => {
    if (!context.auth?.uid) {
      throw new functions.https.HttpsError('unauthenticated', 'Đăng nhập để tạo nhà.');
    }
